...
/**
 * This class's methods are used to extract data from the 6-bit packed ASCII string used by
 * AIVDM/AIVDO AIS messages.
 *
 * <p>init() should be called with a sixbit ASCII string.
 *
 * <p>Up to 32 bits of data are fetched from the string by calling get()
 *
 * <p>Use padBits() to set the number of padding bits at the end of the message, it defaults to 0 if
 * not set.
 */
public class Sixbit {
     ...
     *
     * <p>This function initializes the state of the sixbit parser variables
     */
     ...
     *
     * <p>This function checks the ASCII value to make sure it can be converted. If not, it throws
     * an IllegalArgumentException Otherwise it returns the 6-bit binary value.
     *
     * @param ASCII character to convert
     *     <p>This is used to convert the packed 6-bit value to a binary value. It is not used to
     *     convert data from fields such as the name and destination -- Use ais2ascii() instead.
     */
    public int binfrom6bit(int ascii) throws IllegalArgumentException {
        if ((ascii < 0x30) || (ascii > 0x77) || ((ascii > 0x57) && (ascii < 0x60)))
            throw new IllegalArgumentException("Illegal 6-bit ASCII value");
        if (ascii < 0x60) return (ascii - 0x30) & 0x3F;
        else return (ascii - 0x38) & 0x3F;
    }

    /**
     * Convert a binary value to a 6-bit ASCII value
     *
     * <p>This function checks the binary value to make sure it can be converted. If not, it throws
     * an IllegalArgumentException. Otherwise it returns the 6-bit ASCII value.
     *
     * @param value to convert
     * @return 6-bit ASCII
     */
    public int binto6bit(int value) throws IllegalArgumentException {
        if (value > 0x3F) throw new IllegalArgumentException("Value is out of range (0-0x3F)");
        if (value < 0x28) return value + 0x30;
        else return value + 0x38;
    }
     ...
     *     <p>return - corresponding ASCII value (0x20-0x5F)
     *     <p>This function is used to convert binary data to ASCII. This is different from the
     *     6-bit ASCII to binary conversion for VDM messages; it is used for strings within the
     *     datastream itself. eg. Ship Name, Callsign and Destination.
     */
    public int ais2ascii(int value) throws IllegalArgumentException {
        if (value > 0x3F) throw new IllegalArgumentException("Value is out of range (0-0x3F)");
        if (value < 0x20) return value + 0x40;
        else return value;
    }
     ...
     *     <p>This method returns the requested number of bits to the caller. It pulls the bits from
     *     the raw 6-bit ASCII as they are needed.
     */
    public long get(int numbits) throws SixbitsExhaustedException {
        long result;
        int fetch_bits;
        ...
        while (fetch_bits > 0) {
            ...
            if (this.bitsIndex < this.bits.length()) {
                ...
                this.bitsIndex++;
                if (this.bitsIndex == this.bits.length()) this.remainderLength = 6 - this.padBits;
                else this.remainderLength = 6;
     ...
     * @return String of the characters
     */
